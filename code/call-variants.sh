...
do
    for rep in 1 2 3
    do
        samtools fixmate -O bam ${aln}${grp}${rep}.sam | samtools sort -O bam -o ${aln}${grp}${rep}.bam
        samtools index ${aln}${grp}${id}.bam
    done
    bcftools mpileup -Ob -Q 20 -f ${index} -s ${grp}1.bam,${grp}2.bam,${grp}3.bam -o ${variants}${grp}_raw.bcf
    bcftools call -m -Ov ${variants}${grp}_raw.bcf | bcftools filter -Ov -i '%QUAL>20' -o ${variants}${grp}.vcf
